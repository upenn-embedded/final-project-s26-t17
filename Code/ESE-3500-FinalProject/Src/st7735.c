...
/* -------------------- Control pins -------------------- */
/*
 * SPI bus pins come from spi.c:
 * PA5 = SCK
 * PA6 = MISO
 * PA7 = MOSI
 *
 * This display driver owns its own control pins:
 * PA9  = CS
 * PA8  = D/C
 * PA10 = RST
 */
#define ST7735_CS_PORT   GPIOA
#define ST7735_CS_PIN    9U
...
static void st7735_gpio_init(void)
{
    ...
    /* CS pin as output */
    ST7735_CS_PORT->MODER &= ~(3U << (ST7735_CS_PIN * 2));
    ST7735_CS_PORT->MODER |=  (1U << (ST7735_CS_PIN * 2));
    ...
    /* Push-pull outputs */
    ST7735_CS_PORT->OTYPER  &= ~(1U << ST7735_CS_PIN);
    ST7735_DC_PORT->OTYPER  &= ~(1U << ST7735_DC_PIN);
    ST7735_RST_PORT->OTYPER &= ~(1U << ST7735_RST_PIN);

    /* Default states */
    ST7735_CS_PORT->ODR  |= (1U << ST7735_CS_PIN);
    ST7735_DC_PORT->ODR  |= (1U << ST7735_DC_PIN);
    ST7735_RST_PORT->ODR |= (1U << ST7735_RST_PIN);
}

static void st7735_cs_low(void)
{
    ST7735_CS_PORT->ODR &= ~(1U << ST7735_CS_PIN);
}

static void st7735_cs_high(void)
{
    ST7735_CS_PORT->ODR |= (1U << ST7735_CS_PIN);
}
...
void st7735_init(void)
{
    ...
    /*
     * MADCTL:
     * orientation / memory access control
     */
    st7735_write_command(ST7735_MADCTL);
    st7735_write_data8(0xA0);
